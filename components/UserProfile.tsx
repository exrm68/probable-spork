...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  // ── Settings load — getDoc + onSnapshot দুটোই ──────────────────────────
  const applySettings = (d: any) => {
    const rBot = String(d.referralBotUsername || '').replace('@','').replace('/','').trim();
    const rApp = String(d.referralAppName || '').replace('/','').replace('@','').trim();
    setCs({
  ...
  // ── Referral link — Admin Panel Settings থেকে আসা value ব্যবহার করো ──
  const referralLink = React.useMemo(() => {
    if (!userData?.referralCode) return '';
    const botName = (cs.referralBotUsername || '').replace('@','').trim();
    const appName = (cs.referralAppName || '').replace('/','').trim();
    if (!botName) return '';
    return appName
      ? `https://t.me/${botName}/${appName}?startapp=ref_${userData.referralCode}`
      : `https://t.me/${botName}?start=ref_${userData.referralCode}`;
